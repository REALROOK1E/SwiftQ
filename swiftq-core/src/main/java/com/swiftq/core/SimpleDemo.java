...
public class SimpleDemo {
    ...
    private static void demoStateMachine() {
        System.out.println("\n--- 状态机演示 ---");
        MessageStateMachine stateMachine = new MessageStateMachine();
        
        // 创建消息
        Message msg = new Message("TEST", "Hello State Machine");
        System.out.println("初始状态: " + msg.getState());
        
        // 状态转换: INIT -> SENDING
        boolean success = stateMachine.transitionState(msg, StateEvent.SEND);
        System.out.println("发送事件 -> " + (success ? "成功" : "失败") + ", 新状态: " + msg.getState());
        
        // 状态转换: SENDING -> SENT
        success = stateMachine.transitionState(msg, StateEvent.SENT);
        System.out.println("已发送事件 -> " + (success ? "成功" : "失败") + ", 新状态: " + msg.getState());
        
        // 状态转换: SENT -> CONFIRMED
        success = stateMachine.transitionState(msg, StateEvent.CONFIRM);
        System.out.println("确认事件 -> " + (success ? "成功" : "失败") + ", 新状态: " + msg.getState());
        
        // 尝试无效转换
        success = stateMachine.transitionState(msg, StateEvent.RETRY);
    ...
    private static void demoMultiIndex() {
        ...
        // 创建测试消息
        Message msg1 = new Message("ORDER", "Order #1");
        msg1.setTags(Arrays.asList("urgent", "payment"));
        msg1.setPriority(10);
        
        Message msg2 = new Message("NOTIFY", "Notification #1");
        msg2.setTags(Arrays.asList("normal", "email"));
        msg2.setPriority(5);
        
        Message msg3 = new Message("ORDER", "Order #2");
        msg3.setTags(Arrays.asList("urgent", "shipping"));
        msg3.setPriority(8);
        ...
        // 按topic查询
        List<Message> orderMessages = index.queryByTopic("ORDER");
        System.out.println("ORDER topic消息数: " + orderMessages.size());
        
        // 按tag查询
        List<Message> urgentMessages = index.queryByTag("urgent");
        System.out.println("urgent tag消息数: " + urgentMessages.size());
        
        // 按优先级范围查询
        List<Message> highPriorityMessages = index.queryByPriorityRange(8, 15);
    ...
    private static void demoRouter() {
        ...
        // 添加路由规则
        router.addRule(new TagPriorityRule("urgent", 10, "urgent-queue"));
        router.addRule(new TopicRule("ORDER", "order-queue"));
        router.addRule(new TopicRule("NOTIFY", "notify-queue"));
        
        // 创建测试消息
        Message urgentOrder = new Message("ORDER", "Urgent order");
        urgentOrder.setTags(Arrays.asList("urgent"));
        urgentOrder.setPriority(10);
        
        Message normalNotify = new Message("NOTIFY", "Normal notification");
        normalNotify.setPriority(5);
        
        Message regularOrder = new Message("ORDER", "Regular order");
        regularOrder.setPriority(3);
        
        // 路由测试
        List<String> queues1 = router.route(urgentOrder);
        System.out.println("紧急订单路由到: " + queues1);
        
        List<String> queues2 = router.route(normalNotify);
        System.out.println("普通通知路由到: " + queues2);
        
        List<String> queues3 = router.route(regularOrder);
